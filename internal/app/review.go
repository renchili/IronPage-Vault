...
type redactionListRow struct { ID string `db:"id" json:"id"`; DocumentID string `db:"document_id" json:"document_id"`; Page int `db:"page" json:"page"`; Status string `db:"status" json:"status"`; CreatedBy string `db:"created_by" json:"created_by"`; CreatedAt string `db:"created_at" json:"created_at"` }
...
func (a *App) listRedactions(c echo.Context) error {
	...
	if err := a.db.SelectContext(c.Request().Context(), &rows, `SELECT id,document_id,page,status,created_by,created_at::text AS created_at FROM redaction_proposals WHERE document_id=$1 ORDER BY created_at DESC`, c.Param("id")); err != nil { return apiErr(c, http.StatusInternalServerError, "REDACTION_QUERY_ERROR", "could not list redactions") }
...
func (a *App) confirmRedaction(c echo.Context) error {
	...
	docID := c.Param("id")
	d, err := a.ensureMutable(c, docID)
	if err != nil { return mutableError(c, err) }
	...
	if err != nil { return apiErr(c, http.StatusNotFound, "VERSION_NOT_FOUND", "current version not found") }
	...
	if err != nil { return apiErr(c, http.StatusInternalServerError, "REDACTION_QUERY_ERROR", "could not load redaction regions") }
	if _, err := service.ApplyRedactionBurnIn(v.FilePath, dst, regions); err != nil { return apiErr(c, http.StatusInternalServerError, "REDACTION_BURNIN_ERROR", "could not create redacted binary") }
	...
	if err := a.insertAuditRecord(c.Request().Context(), p.UserID, "REDACTION_CONFIRM", docID, currentRequestID(c), c.RealIP(), nil); err != nil { return apiErr(c, http.StatusInternalServerError, "AUDIT_CREATE_ERROR", "could not record audit log") }
...
func (a *App) applyBates(c echo.Context) error { return a.applyBatesVersion(c) }
