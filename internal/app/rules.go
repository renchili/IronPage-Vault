...
func IsValidRedactionRegion(page int, width float64, height float64) bool { return core.IsValidRedactionRegion(page, width, height) }
func IsValidBatchSize(count int, max int) bool { return core.IsValidBatchSize(count, max) }
func IsRequestTimestampFresh(now time.Time, requestTime time.Time, maxAge time.Duration) bool { return core.IsRequestTimestampFresh(now, requestTime, maxAge) }
func ShouldLockAfterFailedAttempt(failedAttemptsAfterIncrement int) bool { return core.ShouldLockAfterFailedAttempt(failedAttemptsAfterIncrement) }
func IsAccountLocked(now time.Time, lockedUntil *time.Time) bool { return core.IsAccountLocked(now, lockedUntil) }
