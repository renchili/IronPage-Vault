...
func (a *App) login(c echo.Context) error {
    ...
    if err != nil { return apiErr(c, http.StatusInternalServerError, "DB_READ_ERROR", "could not read user") }
    ...
    if err != nil { return apiErr(c, http.StatusInternalServerError, "TOKEN_SIGN_ERROR", "could not sign token") }
    ...
    if err != nil { return apiErr(c, http.StatusInternalServerError, "SESSION_CREATE_ERROR", "could not create session") }
...
func (a *App) authMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
    return func(c echo.Context) error {
        ...
        if !strings.HasPrefix(raw, "Bearer ") { return apiErr(c, http.StatusUnauthorized, "AUTH_REQUIRED", "bearer token is required") }
        ts := c.Request().Header.Get("X-Request-Timestamp")
        if ts == "" { return apiErr(c, http.StatusBadRequest, "REQUEST_TIMESTAMP_REQUIRED", "X-Request-Timestamp is required") }
        parsed, err := time.Parse(time.RFC3339, ts)
        if err != nil { return apiErr(c, http.StatusBadRequest, "REQUEST_TIMESTAMP_INVALID", "timestamp must be RFC3339") }
        ...
        if err != nil || !token.Valid { return apiErr(c, http.StatusUnauthorized, "TOKEN_INVALID", "token is invalid") }
        jti, _ := claims["jti"].(string)
        sub, _ := claims["sub"].(string)
        role, _ := claims["role"].(string)
        username, _ := claims["username"].(string)
        if jti == "" || sub == "" { return apiErr(c, http.StatusUnauthorized, "TOKEN_INVALID", "token claims are incomplete") }
        var blacklisted int
        a.db.GetContext(c.Request().Context(), &blacklisted, `SELECT COUNT(*) FROM jwt_blacklist WHERE jti=$1`, jti)
        if blacklisted > 0 { return apiErr(c, http.StatusUnauthorized, "TOKEN_REVOKED", "token has been revoked") }
        ...
        if err != nil { return apiErr(c, http.StatusInternalServerError, "REPLAY_GUARD_ERROR", "could not record request id") }
        rows, _ := res.RowsAffected()
        if rows == 0 { return apiErr(c, http.StatusConflict, "REPLAY_DETECTED", "request id was already used") }
        var active int
        ...
        if active == 0 { return apiErr(c, http.StatusUnauthorized, "SESSION_EXPIRED", "session is expired or revoked") }
