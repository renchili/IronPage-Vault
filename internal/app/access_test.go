...
func TestCanReadDocumentObject(t *testing.T) {
	...
	if canReadDocumentObject(Principal{Role: RoleReviewer, UserID: "rev1"}, draft) {
		t.Fatalf("reviewer should not read draft")
	}
	if !canReadDocumentObject(Principal{Role: RoleReviewer, UserID: "rev1"}, review) {
...
func TestCanEditDocumentObject(t *testing.T) {
	...
	if canEditDocumentObject(Principal{Role: RoleAdmin, UserID: "admin"}, d) {
		t.Fatalf("admin is not editor for document mutation")
	}
	if canEditDocumentObject(Principal{Role: RoleReviewer, UserID: "rev1"}, d) {
		t.Fatalf("reviewer should not edit document")
	}
...
func TestCanReviewDocumentObject(t *testing.T) {
	if canReviewDocumentObject(Principal{Role: RoleReviewer}, Document{Status: StatusDraft}) {
		t.Fatalf("draft should not be reviewable")
	}
	if !canReviewDocumentObject(Principal{Role: RoleReviewer}, Document{Status: StatusUnderReview}) {
		t.Fatalf("under review should be reviewable")
	}
	if canReviewDocumentObject(Principal{Role: RoleReviewer}, Document{Status: StatusFinalized}) {
		t.Fatalf("finalized should not be reviewable")
	}
	if canReviewDocumentObject(Principal{Role: RoleEditor}, Document{Status: StatusUnderReview}) {
...
func TestCanTransitionDocumentObject(t *testing.T) {
	...
	if !canTransitionDocumentObject(Principal{Role: RoleEditor, UserID: "editor1"}, ownedDraft) {
		t.Fatalf("owner editor should transition own draft")
	}
	...
	if canTransitionDocumentObject(Principal{Role: RoleReviewer, UserID: "rev1"}, ownedDraft) {
		t.Fatalf("reviewer should not transition draft")
	}
	review := Document{OwnerID: "editor1", Status: StatusUnderReview}
	if !canTransitionDocumentObject(Principal{Role: RoleReviewer, UserID: "rev1"}, review) {
		t.Fatalf("reviewer should transition reviewable document")
	}
	finalized := Document{OwnerID: "editor1", Status: StatusFinalized}
	if canTransitionDocumentObject(Principal{Role: RoleEditor, UserID: "editor1"}, finalized) {
