...
func (a *App) runScheduledBackup(ctx context.Context) error {
	...
	_, _ = a.db.ExecContext(ctx, `INSERT INTO audit_logs(id,actor_user_id,action_type,request_id,source_ip,metadata,created_at) VALUES($1,NULL,'SCHEDULED_BACKUP_CREATE',$2,'scheduler',$3::jsonb,NOW())`, makeIdentifier("aud"), "scheduler", `{"database_dump_mode":"`+artifacts.DatabaseDumpMode+`","file_snapshot_mode":"`+artifacts.FileSnapshotMode+`"}`)
	return nil
}
