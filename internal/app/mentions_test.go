...
func TestExtractMentionUsernamesIgnoresNonMentions(t *testing.T) {
    ...
    if len(got) != 0 { t.Fatalf("unexpected mentions: %#v", got) }
}
...
func TestExtractMentionUsernamesTrimsPunctuation(t *testing.T) {
    ...
    if len(got) != 2 { t.Fatalf("expected 2 mentions, got %#v", got) }
    if got[0] != "reviewer-one" || got[1] != "legal.team" { t.Fatalf("unexpected mentions: %#v", got) }
}
