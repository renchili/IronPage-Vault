...
func (a *App) runBackupMetadataSnapshot(c echo.Context) error {
    ...
    if err != nil { return apiErr(c, http.StatusInternalServerError, "BACKUP_SNAPSHOT_ERROR", "could not collect backup metadata snapshot") }
    raw, err := json.MarshalIndent(snapshot, "", "  ")
    if err != nil { return apiErr(c, http.StatusInternalServerError, "BACKUP_ENCODE_ERROR", "could not encode backup metadata snapshot") }
    if err := os.WriteFile(target, raw, 0640); err != nil { return apiErr(c, http.StatusInternalServerError, "BACKUP_WRITE_ERROR", "could not write backup metadata snapshot file") }
    ...
    if err != nil { return apiErr(c, http.StatusInternalServerError, "BACKUP_CREATE_ERROR", "could not record backup metadata snapshot job") }
