...
BASE_COMPARE_VERSION="$(json_field data[0].id)"
if [ -z "$BASE_COMPARE_VERSION" ]; then echo "FAIL api: left version id missing"; exit 1; fi
...
NEW_COMPARE_VERSION="$(json_field data[0].id)"
if [ -z "$NEW_COMPARE_VERSION" ]; then echo "FAIL api: right version id missing"; exit 1; fi
