...
check "rule entrypoint roles are unambiguous" "grep -q 'mandatory repository entrypoint' AGENTS.md && grep -q 'project-adapted rule file' AGENTS.md && grep -q '# AGENT Rules for IronPage Vault' AGENT.md"
check "rule discovery tolerates case singular plural and glob spelling" "grep -q 'Tolerant rule-path resolution' AGENTS.md && grep -q 'case-insensitively' AGENTS.md && grep -q 'singular/plural' AGENTS.md && grep -q 'skill/\*\*/skill.md' AGENTS.md && grep -q 'A glob is not a loaded file' skills/full-project-acceptance-hard-gates/SKILL.md"
check "rule references cannot become filename nitpicks" "grep -q 'do not stop, fail, ask the user' AGENTS.md && grep -q 'Do not fail for harmless rule-path capitalization' skills/full-project-acceptance-hard-gates/SKILL.md && grep -q 'only report a rule as missing after tolerant discovery' skills/project-generation-workflow/SKILL.md"
check "generation is static and externally independent" "grep -q 'static source-completion tasks' skills/project-generation-workflow/SKILL.md && grep -q 'trigger, rerun, retry, dispatch, approve, cancel, or wait for CI' skills/project-generation-workflow/SKILL.md && grep -q 'CI triggered, cancelled, or awaited:' skills/project-generation-workflow/SKILL.md"
check "offline generation cannot fall back to execution" "grep -q 'No-execution fallback and connectivity rule' skills/project-generation-workflow/SKILL.md && grep -q 'No network, unavailable internet' skills/project-generation-workflow/SKILL.md && grep -q 'temporary workflow, temporary execution branch' skills/project-generation-workflow/SKILL.md && grep -q 'Ambiguous test wording is not execution permission' skills/project-generation-workflow/SKILL.md"
...
check "acceptance remains non-executing without network or tools" "grep -q 'Offline and missing-tool behavior' skills/full-project-acceptance-hard-gates/SKILL.md && grep -q 'No network or missing tooling never authorizes execution' skills/full-project-acceptance-hard-gates/SKILL.md && grep -q 'create a temporary workflow' skills/full-project-acceptance-hard-gates/SKILL.md && grep -q 'Ambiguous test wording is not execution permission' skills/full-project-acceptance-hard-gates/SKILL.md"
...
check "obsolete execution-gated acceptance rules are absent" "! grep -Fq 'Missing execution evidence must be recorded as' skills/full-project-acceptance-hard-gates/SKILL.md && ! grep -Fq 'PASS requires an executed generated summary' skills/full-project-acceptance-hard-gates/SKILL.md"
