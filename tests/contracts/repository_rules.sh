...
check "workflow paginates complete history" "grep -q 'github.paginate' .github/workflows/ci.yml && grep -q 'listWorkflowRunsForRepo' .github/workflows/ci.yml"
...
check "source inventory audits generic path hazards" "grep -q 'non-ASCII tracked path' ci/source_inventory.py && grep -q 'case-only path collision' ci/source_inventory.py && grep -q 'near-duplicate sibling paths' ci/source_inventory.py && grep -q 'mixed hyphen/underscore naming' ci/source_inventory.py"
