...
check "restore stages files and records lifecycle" "grep -q 'unsafe path in filesystem snapshot' internal/platform/backup_exec.go && grep -q -- '--single-transaction' internal/platform/backup_exec.go && grep -q 'requested_by_user_id' internal/app/restore.go && grep -q 'BACKUP_RESTORE_COMPLETED' internal/app/restore.go"
