...
check "workflow definitions are Admin-managed and runtime-resolved" "grep -q 'PUT(\"/workflow-statuses\"' internal/app/server.go && grep -q 'replaceWorkflowStatuses' internal/app/workflow_definitions.go && grep -q 'nextWorkflowDefinition' internal/app/workflows.go"
...
check "Bates range, version, document, and audit share a transaction" "grep -q 'AllocateBatesRange' internal/app/bates_version.go internal/repository/bates.go && grep -q 'auditWithExecutor(c, tx' internal/app/bates_version.go && grep -q 'os.Remove(dst)' internal/app/bates_version.go"
