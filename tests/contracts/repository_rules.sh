...
check "generation is static and externally independent" "grep -q 'static source-completion tasks' skills/project-generation-workflow/SKILL.md && grep -q 'trigger, rerun, retry, dispatch, approve, cancel, or wait for CI' skills/project-generation-workflow/SKILL.md && grep -q 'CI triggered or awaited: \`none\`' skills/project-generation-workflow/SKILL.md"
check "generation rejects narrow examples and blocked-operation bypass" "grep -q 'A user-provided example reveals a general capability or quality gap' skills/project-generation-workflow/SKILL.md && grep -q 'Absolute blocker and no-bypass rule' skills/project-generation-workflow/SKILL.md && grep -q 'Any execution blocker immediately stops the blocked operation' skills/project-generation-workflow/SKILL.md && grep -q 'switch to another account, identity, repository, branch, tool, connector, API, or execution environment to evade the blocker' skills/project-generation-workflow/SKILL.md"
check "generation requires implementation-ready UI decisions" "grep -q 'User-interface design and implementation contract' skills/project-generation-workflow/SKILL.md && grep -q 'Separate environment-owned and product-owned UI' skills/project-generation-workflow/SKILL.md && grep -q 'exact icon source and icon identity' skills/project-generation-workflow/SKILL.md && grep -q 'Interaction state-machine contract' skills/project-generation-workflow/SKILL.md && grep -q 'Editable prototype contract' skills/project-generation-workflow/SKILL.md"
check "generation preserves artifact and documentation ownership" "grep -q 'A requested report or prototype does not automatically authorize adding it to the repository' skills/project-generation-workflow/SKILL.md && grep -q 'Do not create fixed default documentation paths in this Skill' skills/project-generation-workflow/SKILL.md"
...
check "restore stages files and preserves acting user" "grep -q 'unsafe path in filesystem snapshot' internal/platform/backup_exec.go && grep -q -- '--single-transaction' internal/platform/postgres_command.go && grep -q 'ActorUserID:.*p.UserID' internal/app/restore.go && grep -q 'BACKUP_RESTORE_COMPLETED' internal/app/restore.go"
check "restore lifecycle preserves interrupted outcomes" "grep -q 'restoreStatusInterrupted' internal/app/restore_lifecycle.go && grep -q 'operator_verification_required' internal/app/restore_lifecycle.go && grep -q 'resolveInterruptedRestore' internal/app/server.go internal/app/restore.go && ! grep -A8 'record.Status == restoreStatusRequested' internal/app/restore_lifecycle.go | grep -q 'restoreStatusFailed'"
...
check "backup restore and configuration integrity" "bash tests/contracts/backup_restore_integrity.sh"
