...
check "pagination contracts cover every collection" "for route in '/api/documents' '/api/admin/users' '/api/admin/config' '/api/admin/workflow-statuses' '/api/admin/notification-templates' '/api/admin/backup/jobs' '/versions' '/redactions' '/annotations' '/api/notifications' '/api/audit-logs'; do grep -Fq \"$route\" tests/api/test_api_contracts.sh || exit 1; done && grep -q 'page_size=0' tests/api/test_api_contracts.sh && grep -q 'page_size=1' tests/api/test_api_contracts.sh && grep -q 'page_size=101' tests/api/test_api_contracts.sh"
