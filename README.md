...
This is a **pure backend project**. The browser UI under `public/` is a backend acceptance testing aid for manual API probing and automated screenshot evidence. It is not a formal frontend deliverable, not part of the product scope, and should not be evaluated as a fullstack application.
...
public/              backend test UI only, with screenshot evidence generated by run_tests.sh
...
| Testing | Docker-based acceptance, API scripts, unit checks, local PDF/CSV fixtures, and automated UI screenshot evidence |
...
API_tests/test_ui_screenshot_acceptance.sh
unit_tests/test_structure_rules.sh
API_tests/test_strict_dependency_failures.sh
API_tests/test_bates_sequence_multi_doc.sh
```

`API_tests/test_ui_screenshot_acceptance.sh` opens `/ui/` with headless Chrome/Chromium and writes screenshot evidence to `artifacts/local-acceptance/ui/`. The main `artifacts/local-acceptance/report.html` embeds the generated screenshot when this stage succeeds.
