...
check "backup metadata paths are asserted" "grep -q 'expect_json_nonempty.*artifacts.database_dump_path' API_tests/test_admin_ops.sh && grep -q 'expect_json_nonempty.*artifacts.file_snapshot_path' API_tests/test_admin_ops.sh"
check "backup restore consumes returned artifact paths" "grep -q 'database_dump_path.*DB_DUMP_PATH' API_tests/test_admin_ops.sh && grep -q 'file_snapshot_path.*FILE_SNAPSHOT_PATH' API_tests/test_admin_ops.sh"
